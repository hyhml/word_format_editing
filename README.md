...
- `content.template` 使用字段、字面量、空格、制表位、引导符和结构化 `choice` token 描述编号与内容模板；`choice` 仅表示规范明确允许的多个选项，不用于掩盖识别歧义。
...
武汉大学本科规范测试进一步覆盖了封面字段、中英文扉页、声明正文、结论、目录编号片段、图表子标题、附录编号、符号分类和九类参考文献独立模板。所有扩展仍落入唯一的目标—属性结构，后续执行器无需重新解释自然语言。

复旦大学规范测试补充了整节目录/摘要/正文目标、目录页码片段、论文与设计条件封面、含公式段落的条件行距和字符单位悬挂缩进。完整块分类会屏蔽示例、行政说明和链接中的正则误命中；AI 对同一证据作出的精确作用域判断可以替换粗粒度正则判断，但不同证据之间仍按冲突处理。

编译器只统计 Word 格式问题。内容质量、本人签名、提交手续、打印和实体装订不进入格式未解决项。页眉等动态文字可以使用 `degree_name` 等模板字段，待后续论文结构识别从目标 Word 确定论文类型；运行时输入无法确定时保持原文字。

仓库内的 `resolve-format-runtime-context` skill 负责按需解析这些动态字段。它没有新增 CLI：skill 通过 `runtime_context.py` 构造最小证据请求，由 AI 只判断 `format_spec` 实际需要的固定枚举，再校验文档指纹、字段范围、证据 ID 和置信度。当前支持从目标 DOCX 判断 `document.degree_type` 以及本科毕业论文/毕业设计对应的 `document.project_type`；它们可分别驱动页眉模板和条件封面，失败或不确定时不修改原文字。
