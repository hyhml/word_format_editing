...
- [x] 校验表格格式。
- [x] 校验图表题格式。
- [x] 校验页眉页脚距离；复杂页眉页脚内容先以 warn 记录。
- [x] 给出 pass/warn/fail 三级结果。
...
- [x] 定义 `format_spec` Schema v2。
- [x] 建立论文对象词表和格式属性词表。
- [x] 使用 `set` / `preserve` / `remove` 统一属性动作。
- [x] 建立 TXT/Markdown/JSON/DOCX/PDF 统一来源块模型并保留位置证据。
- [x] 分离格式要求抽取正则与论文对象定位正则。
- [x] 实现中文字号、厘米/毫米/英寸、磅值和行距规整。
- [x] 实现唯一性合并：未解决冲突在正式规范中转换为 `preserve`。
- [x] 实现 Schema、数值、单位、正则、对象和属性校验。
- [x] 校验失败时生成机器可读的 `repair_request`。
- [x] 拒绝 AI 候选引用不存在的来源证据。
- [x] 输出 `format_spec.json`、`recognition_report.json` 和 `recognition_report.md`。
- [x] 为旧格式化引擎增加 Schema v2 兼容门控，避免误执行。
- [x] 编写约束 AI 编译和最多三轮定点修复的项目 Skill。
- [ ] 使用真实论文格式要求建立端到端验收 fixture。
...
- [x] 自动创建或复用格式包。
- [x] 输出格式化 Word 和报告。
- [x] 串联模块 0 到模块 5，形成 `format_pipeline.py run` 入口命令。
- [x] 输出 `formatted.docx`、结构报告、格式化报告、工作流报告、校验报告和 pipeline 汇总报告。

### M6：OpenXML 高级补丁

- [x] 从 `format_thesis.py` 中拆出可复用补丁第一版。
- [x] 支持三线表、页眉页脚、图表题、参考文献、数学字体和公式段落对齐。
- [x] 为 `openxml_patches/` 建立补丁级测试样例。
- [x] 在模块 5 中补充高级补丁对应的合规校验第一版。
- [ ] 增强复杂公式编号：矩阵、多行公式和 Word 原生 `eqArr` 重排。
...
M1 到 M6 的第一版已完成。当前优先完成 M6.5：以 Schema v2 为唯一、规整的格式要求识别产物，后续格式化引擎适配暂不在本阶段进行。

M6.5 完成后的顺序：

1. 使用真实论文格式要求验证对象和属性覆盖率。
2. 根据真实样例补充要求抽取模式和 AI Skill 约束。
3. 再启动格式化引擎对 Schema v2 的适配。
4. 复杂公式编号仍保留为后续专项。

复杂公式编号增强的第一批任务：

1. 从 `format_thesis.py` 的 `fix_equation_alignment()` 中拆出可复用的 `eqArr` 转换逻辑。
2. 覆盖普通公式、矩阵公式和多行公式的 fixture。
3. 在模块 5 中补充 `eqArr`、右侧编号和多行公式布局校验。
4. 明确不能自动修复的公式场景，并在报告中给出可人工处理的位置。
