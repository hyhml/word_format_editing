...
- [ ] 实现格式文件哈希计算：单文件 hash 和多文件组合 hash。
- [ ] 实现 `formats/` 格式包索引扫描。
- [ ] 实现精确匹配：`combined_source_hash` 命中即复用。
- [ ] 实现元数据匹配：学校、期刊、年份、关键词。
- [ ] 预留语义相似匹配接口，先不强依赖 embedding。
- [ ] 未提供用户描述且解析信心不足时，输出需要澄清的问题。
...
- [ ] 实现 `.docx` 基础段落读取。
- [ ] 实现标题层级识别：基于 Word 样式、编号模式和文本模式。
- [ ] 实现图题、表题识别。
- [ ] 实现参考文献区段识别。
- [ ] 输出 `paper_structure.md` 供人工阅读。
- [ ] 输出 `paper_structure.json` 供模块 4 使用。
- [ ] 对不能识别的复杂对象标记为 `preserve`，后续保留原样。
...
- 建立 `formats/` 目录规范。
- 实现格式文件 hash 和 manifest 匹配。
- 能判断“复用旧格式包”还是“创建新格式包”。
...
- 从 `raw.docx` 输出 `paper_structure.md/json`。
- 支持标题、图题、表题、参考文献基础识别。
