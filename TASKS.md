...
M1、M2、M3、M4（论文结构识别）、模块 4（工作流门控与 formatter 启动）和模块 5（格式合规性校验与报告）已完成。下一步建议推进 M5：端到端最小闭环。

端到端最小闭环的第一批任务：

1. 串联模块 0 到模块 5，形成一个入口命令。
2. 输入格式要求文件集和原始论文 `raw.docx`。
3. 自动复用或创建格式包。
4. 输出 `formatted.docx`、格式化报告、工作流报告和校验报告。
