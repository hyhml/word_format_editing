...
description: Resolve only the runtime variables that a canonical format_spec needs from the target paper DOCX, such as degree type for a running-header template or thesis versus design for a conditional cover. Use after format requirements have been compiled when format_spec contains template or condition runtime inputs and a target Word file is available. Ask AI for a bounded, evidence-backed classification; do not review, rewrite, summarize, or judge paper content, and do not format the document.
---
...
1. Load the canonical `format_spec.json` and call `required_runtime_inputs(spec)`. It scans both structured template fields and conditional cases. If it returns no AI-resolved fields, stop successfully without reading the paper.
...
- Prefer explicit cover, title-page, degree-category, application-degree, project-type, “本科毕业论文”, and “本科毕业设计” statements.
...
- Never infer degree or project type from research topic, department, writing style, author biography, or cited works.
